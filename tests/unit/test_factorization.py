...
# Centralised utilities themselves, or one-off edge cases.
ALLOWED_JAX_IMPORT_FILES = {
    str(LAYER_DIR / "jax_compat.py"),
    str(CORE_DIR / "backends" / "lazy_import.py"),
    str(CORE_DIR / "backends" / "tpu_backend.py"),  # re-exports via ensure_jax
}

ALLOWED_TORCH_IMPORT_FILES = {
    str(CORE_DIR / "backends" / "lazy_import.py"),
    str(CORE_DIR / "backends" / "gpu_backend.py"),  # re-exports via ensure_torch
    str(CORE_DIR / "backends" / "utils.py"),  # detection utility probes availability
}

ALLOWED_SAFE_IMPORT_FILES = {
    str(CORE_DIR / "import_utils.py"),  # the utility itself
}

ALLOWED_ATTENTION_RESHAPE_FILES = {
    str(LAYER_DIR / "attention_utils.py"),  # the utility itself
    str(LAYER_DIR / "sparsity" / "sparse_capibara.py"),  # reshape after jnp.split, not from fused dim
}
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ClassDef):
                ...
                if name.endswith("Wrapper") and name != "CompositeWrapper":
                    # Check if it's a real class def (not a partial alias)
...
class TestFactorizationUtilitiesExist:
    ...
    def test_import_utils_importable(self):
        from core.import_utils import safe_import, safe_import_module  # noqa: F401
