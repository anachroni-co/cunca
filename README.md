...
- This is a research codebase with experimental and hardware-specific components.
- GPU features require PyTorch + CUDA; TPU features require JAX/Flax. CPU-only works with base deps.
- Some advanced modules (e.g., VQ/JAX-native) are optional and may be unavailable without those deps.
- Reported benchmarks/coverage are snapshots and may not match your environment; re-run tests to confirm.
- CLI coverage is minimal (`capibara --info/--health/--demo`); training/inference CLIs are pending.
